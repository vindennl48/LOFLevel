...
//    label_btn_reset("Standby..")
{
    ...
    windowSwitcher.switchTo("main");
    addAndMakeVisible(windowSwitcher);
    
    DBG(windowSwitcher.activeWindow);
...
{
    if (windowSwitcher.activeWindow == "sub")
        windowSwitcher.switchTo("main");
    else
        windowSwitcher.switchTo("sub");
...
{
    windowSwitcher.setBounds(0, 0, getWidth(), getHeight());
